      ...
      <header className="bg-blue-700 text-white shadow-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-base sm:text-lg hidden xs:inline">Gestione Magazzino</span>
            <span className="font-bold text-base sm:hidden">Magazzino</span>
        ...
        <nav className="max-w-5xl mx-auto px-4 flex gap-1 pb-0">
          {navItems.map(item => (
            <Link
              key={item.href}
              href={item.href}
              className={`px-3 sm:px-4 py-2 text-sm font-medium rounded-t-lg transition-colors flex items-center gap-1 ${
                pathname === item.href
